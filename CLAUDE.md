...
├── tests/                  # pytest suite (325 tests, uses responses + pytest-flask)
...
- **pytest suite**: 325 tests in `tests/` covering services, routes, and data integrity. Run with `source .venv/bin/activate && pytest` (or `pytest -v` for verbose). Uses `responses` to mock HTTP calls and `pytest-flask` for the test client.
